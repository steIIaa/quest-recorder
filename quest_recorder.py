...
class App:
    def wait_for_file_to_finish(self, filepath):
        """wait until the headset is actually done writing the video file"""
        self.log(f'waiting for {filepath} to finish saving...')

        # tried checking lsof for open file handles first, but android's
        # lsof needs root to see other processes' open fds - without it,
        # the check just comes back empty immediately (false "not open"),
        # which made this finish even faster than before instead of more
        # reliably. dropped that, back to pure size polling but with a
        # hard floor on top so it can never bail out too early no matter
        # what stat reports.
        start_time = time.time()
        previous_size = -1
        unchanged_checks = 0
        required_unchanged = 6   # needs ~24s of zero growth in a row
        minimum_wait_seconds = 15  # never trust "finished" sooner than this

        while True:
            _, output, _ = self.run_adb(['shell', 'stat', '-c', '%s', filepath])
            try:
            ...
            if unchanged_checks >= required_unchanged and elapsed >= minimum_wait_seconds:
                break

            time.sleep(4)

        # let the mp4 finish writing its trailing moov atom metadata
        time.sleep(2)
